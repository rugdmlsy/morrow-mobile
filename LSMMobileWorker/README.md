...
For a physical iPhone with the current Personal Team, build/install the normal `LSMMobileWorker` `Debug` configuration. The Phase 4 app version is `0.4.2`. `mobile.controller_events` is an explicit capability marker; controllers must not deliver event/ACK poll extensions to older iOS workers that do not advertise it.
